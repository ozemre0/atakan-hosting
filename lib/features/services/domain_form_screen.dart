...
class DomainFormScreen extends ConsumerStatefulWidget {
  const DomainFormScreen({
    super.key,
    this.domainId,
  });

  final String? domainId; // null = new, non-null = edit
...
class _DomainFormScreenState extends ConsumerState<DomainFormScreen> {
  ...
  Future<void> _loadDomain() async {
    if (widget.domainId == null) return;
    setState(() => _isLoading = true);
    try {
      final api = ref.read(apiClientProvider);
      final res = await api.getJson('/domains/${widget.domainId}');
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (widget.domainId != null) {
        response = await api.patchJson('/domains/${widget.domainId}', data: data);
      } else {
        response = await api.postJson('/domains', data: data);
      }
      ...
      if (okValue == true || okValue == 'true' || okValue == 1) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(widget.domainId == null ? context.l10n.serviceAdded : context.l10n.serviceUpdated),
  ...
  Widget build(BuildContext context) {
    final isEdit = widget.domainId != null;

    return Scaffold(
      appBar: AppHeader(
        title: Text(isEdit ? context.l10n.editDomain : context.l10n.newDomain),
      ),
      body: Center(
        child: ConstrainedBox(
