...
class HostingFormScreen extends ConsumerStatefulWidget {
  const HostingFormScreen({
    super.key,
    this.hostingId,
  });

  final String? hostingId; // null = new, non-null = edit

  @override
  ConsumerState<HostingFormScreen> createState() => _HostingFormScreenState();
}

class _HostingFormScreenState extends ConsumerState<HostingFormScreen> {
  ...
  Future<void> _loadHosting() async {
    if (widget.hostingId == null) return;
    setState(() => _isLoading = true);
    try {
      final api = ref.read(apiClientProvider);
      final res = await api.getJson('/hostings/${widget.hostingId}');
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (widget.hostingId != null) {
        response = await api.patchJson('/hostings/${widget.hostingId}', data: data);
      } else {
        response = await api.postJson('/hostings', data: data);
      }
      ...
      if (okValue == true || okValue == 'true' || okValue == 1) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(widget.hostingId == null ? context.l10n.serviceAdded : context.l10n.serviceUpdated),
            backgroundColor: Colors.green,
            duration: const Duration(seconds: 2),
          ),
        );
        // Navigate to hostings list
        if (mounted) {
          context.go('/hostings');
        }
        return;
  ...
  Widget build(BuildContext context) {
    final isEdit = widget.hostingId != null;

    return Scaffold(
      appBar: AppHeader(
        title: Text(isEdit ? context.l10n.editHosting : context.l10n.newHosting),
      ),
      body: Center(
        child: ConstrainedBox(
